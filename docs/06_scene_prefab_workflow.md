...
10. 主角、城墙线、敌人视觉模板：已完成第一轮场景内战斗表现节点绑定。
...
`BattleMain.scene` 的 `BattleLayer` 下保留：
...
`BattleController` 会优先复用 `MainHeroPrefab` 和 `CityBottomLine`，继续由代码刷新高亮、缩放和受击焦点。`EnemySystem` 会隐藏 `EnemyVisualTemplate`，生成敌人时克隆模板，再按敌人类型刷新头像、血条、名称和状态。正式美术接入时可以把主角身体、头像、城墙线、敌人头像与血条替换成 prefab 子节点，战斗逻辑仍只负责数据刷新和轻量表现。
